...
namespace DesktopSearch.Core.Services
{
    ...
    {
        private readonly ITikaServer _tikaServer;

        public NativeServiceManager(ITikaServer tikaServer)
        {
            _tikaServer = tikaServer;
        }


        public async Task EnsureTikaStarted()
        {
            if (!_tikaServer.IsRunning)
            {
                _tikaServer.Start();
        ...
        public Task CleanupTikaContainers()
        {
            return Task.CompletedTask;
        }
    }
    ...
    {
        private const string localDockerEndpoint = "tcp://10.0.75.1:2375";
